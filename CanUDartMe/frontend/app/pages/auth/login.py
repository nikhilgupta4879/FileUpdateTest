import flet as ft
from app.services.api_client import ApiClient


def build_login_page(page: ft.Page, api: ApiClient, on_success):
    email_field = ft.TextField(label="Email", keyboard_type=ft.KeyboardType.EMAIL)
    password_field = ft.TextField(label="Password", password=True, can_reveal_password=True)
    error_text = ft.Text(color=ft.colors.RED_400, visible=False)

    async def handle_login(e):
        error_text.visible = False
        try:
            resp = await api.login(email_field.value, password_field.value)
            api.set_token(resp["access_token"])
            await on_success()
        except Exception as ex:
            error_text.value = f"Login failed: {ex}"
            error_text.visible = True
        page.update()

    def go_register(e):
        page.go("/register")

    return ft.View(
        "/login",
        controls=[
            ft.AppBar(title=ft.Text("CanUDartMe — Login")),
            ft.Column(
                controls=[
                    ft.Text("Welcome Back!", size=28, weight=ft.FontWeight.BOLD),
                    email_field,
                    password_field,
                    error_text,
                    ft.ElevatedButton("Login", on_click=handle_login, width=300),
                    ft.TextButton("New here? Register", on_click=go_register),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                spacing=16,
            ),
        ],
    )
